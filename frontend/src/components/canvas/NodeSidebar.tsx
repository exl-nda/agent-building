...
import SalesforceOperationConfig from './toolOperations/SalesforceOperationConfig';
import SAPOperationConfig from './toolOperations/SAPOperationConfig';
import WorkdayOperationConfig from './toolOperations/WorkdayOperationConfig';
import DatabricksOperationConfig from './toolOperations/DatabricksOperationConfig';
...
const NodeSidebar = ({ node, onUpdate }: NodeSidebarProps) => {
    ...
    const [literalFields, setLiteralFields] = useState<Array<{
        name: string;
        values: string[];
        description: string;
    }>>([{
    ...
    const prevValuesRef = useRef({
        ...
        outputMode: 'text' as 'text' | 'structured'
    });
    ...
    const handleToolSelect = (tool: Tool) => {
        ...
        // Initialize empty operation config for enterprise tools
        const enterpriseTools = ['salesforce', 'sap', 'workday', 'databricks'];
        const isEnterpriseTool = enterpriseTools.includes(tool.type.toLowerCase());
        ...
        const updatedNode = {
            ...
            data: {
                ...
                // Initialize empty operation config for enterprise tools
                toolOperation: isEnterpriseTool ? {
    ...
    const handleToolOperationUpdate = (updatedNode: CustomNode) => {
        onUpdate(updatedNode);
    };
                            ...
                            {isLoadingProviders ? (
                                ...
                                llmProviders.map((provider) => (
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2 text-sm ${selectedProvider?.alias === provider.alias
                ...
                {!node ? (
                    ...
                    <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="flex space-x-2 mb-4">
                                    <button
                                        ...
                                        className={`flex-1 py-2 px-3 text-sm rounded-lg border ${llmType === 'local'
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 py-2 px-3 text-sm rounded-lg border ${llmType === 'remote'
                                            ? 'bg-purple-600 border-purple-600 text-white'
                                            : 'bg-gray-700 border-gray-600 text-gray-300 hover:bg-gray-600/50'
                                            }`}
                                    >
                            ...
                            <div className="relative">
                                ...
                                <div className="relative">
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between bg-gray-700 border ${currentProvider && !isLoadingModels
                                            ? 'border-gray-600'
                                    ...
                                    >
                                        ...
                                        {!isLoadingModels && (
                                            <FiChevronDown className={`h-4 w-4 text-gray-400 transition-transform ${isModelOpen ? 'transform rotate-180' : ''
                                            ...
                                            <div className="py-1 max-h-60 overflow-auto">
                                                {availableModels.map((model) => (
                                                    <button
                                                        ...
                                                        className={`w-full text-left px-4 py-2 text-sm ${node.data.llm?.model === model.id
                                                            ? 'bg-purple-600 text-white'
                                                            : 'text-gray-300 hover:bg-gray-700'
                                                            }`}
                                                    >
                        ...
                        <div>
                                                ...
                                                {isLoadingTools ? (
                                                    ...
                                                    tools.map((tool) => (
                                                        <button
                                                            ...
                                                            className={`w-full text-left px-4 py-2 text-sm ${node.data.tool?.id === tool.id
                                                                ? 'bg-purple-600 text-white'
                                                                : 'text-gray-300 hover:bg-gray-700'
                                                                }`}
                            ...
                            {/* Enterprise Tool Operations */}
                            {node?.data?.tool?.type && (
                                <>
                                    {node.data.tool.type.toLowerCase() === 'salesforce' && (
                                        <SalesforceOperationConfig
                                            node={node}
                                            onUpdate={handleToolOperationUpdate}
                                        />
                                    )}
                                    {node.data.tool.type.toLowerCase() === 'sap' && (
                                        <SAPOperationConfig
                                            node={node}
                                            onUpdate={handleToolOperationUpdate}
                                        />
                                    )}
                                    {node.data.tool.type.toLowerCase() === 'workday' && (
                                        <WorkdayOperationConfig
                                            node={node}
                                            onUpdate={handleToolOperationUpdate}
                                        />
                                    )}
                                    {node.data.tool.type.toLowerCase() === 'databricks' && (
                                        <DatabricksOperationConfig
                                            node={node}
                                            onUpdate={handleToolOperationUpdate}
                                        />
                                    )}
                                                    ...
                                                    >
                                                        {[
                                                            ...
                                                            <ListboxOption
                                                                key={option.value}
                                                                className={({ active }) =>
                                                                    `relative cursor-default select-none py-2 pl-4 pr-4 ${active ? 'bg-gray-600 text-white' : 'text-gray-300'
                                                                    }`
                                                                }
                                                                value={option.value}
                                                            >
                            ...
                            <div className="mt-4 pt-4">
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-4">
                                    <label className={`relative flex cursor-pointer rounded-lg border-2 p-3 transition-all ${outputMode === 'text'
                                        ...
                                        />
                                        <div className="flex items-center">
                                            <div className={`h-4 w-4 rounded-full border-2 mr-2 flex-shrink-0 transition-all ${outputMode === 'text'
                                    ...
                                    <label className={`relative flex cursor-pointer rounded-lg border-2 p-3 transition-all ${outputMode === 'structured'
                                        ? 'border-purple-500 bg-purple-500/10'
                                        : 'border-gray-600 hover:border-gray-500'
                                        }`}>
                                        <input
                                        ...
                                        />
                                        <div className="flex items-center">
                                            <div className={`h-4 w-4 rounded-full border-2 mr-2 flex-shrink-0 transition-all ${outputMode === 'structured'
                                                ? 'border-purple-400 bg-purple-400 ring-2 ring-purple-400/30'
                                                : 'border-gray-400'
                                                }`}></div>
                                            <div>
