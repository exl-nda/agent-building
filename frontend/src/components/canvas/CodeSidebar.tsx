...
const CodeSidebar = ({ flowId }: CodeSidebarProps) => {
    ...
    const [selectedFramework, setSelectedFramework] = useState<'langgraph' | 'ms-framework'>('langgraph');
    const [selectedEnvironment, setSelectedEnvironment] = useState<'dev' | 'uat' | 'prod'>('dev');
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                <div className="p-4 flex items-center gap-4">
                    <select
                        ...
                        onChange={(e) => setSelectedFramework(e.target.value as 'langgraph' | 'ms-framework')}
                        className="w-full flex-1 py-2 px-3 rounded-md bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    >
                        <option value="langgraph">LangGraph</option>
                        <option value="ms-framework">MS Framework</option>
                    </select>
                    <button
                        onClick={handleGenerateCode}
                        disabled={isGenerating}
                        className={` py-2 px-4 rounded-md text-sm font-medium flex items-center justify-center space-x-2 ${isGenerating
                    ...
                    generatedCode && (
                        <div className="p-4 flex items-center gap-4">
                            <select
                                ...
                                onChange={(e) => setSelectedEnvironment(e.target.value as 'dev' | 'uat' | 'prod')}
                                className="w-full flex-1 py-2 px-3 rounded-md bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            >
